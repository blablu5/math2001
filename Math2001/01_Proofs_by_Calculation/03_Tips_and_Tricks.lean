...
example {u v : ℚ} (h1 : 4 * u + v = 3) (h2 : v = 2) : u = 1 / 4 :=
  calc
    u
      =1/4*(4*u+v)-1/4*v := by ring
    _ =1/4*3-1/4*2 := by rw[h1, h2]
    _ =1/4:= by ring

example {c : ℚ} (h1 : 4 * c + 1 = 3 * c - 2) : c = -3 :=
  calc
    c
      = (4*c + 1) - (3*c -2) - 3 := by ring
...
example {p : ℝ} (h1 : 5 * p - 3 = 3 * p + 1) : p = 2 :=
  calc
    p
      = 1/2*((5 * p - 3) - (3 * p + 1))+2:= by ring
    _ = 1/2*((3 * p + 1) - (3 * p + 1))+2:= by rw[h1]
    _ = 2 := by ring
...
example {a b : ℝ} (h1 : a + 2 * b = 4) (h2 : a - b = 1) : a = 2 :=
  calc
    a
      = 1/3*(2*(a-b)+(a+2*b)) := by ring
    _ = 1/3*(2*1+4) := by rw[h1, h2]
    _ = 2 := by ring
